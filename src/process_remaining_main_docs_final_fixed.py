"""
Final corrected script to process all remaining English documentation files systematically.
This script will properly handle the data structures and create valid JSON files.
"""
...
def create_basic_processed_data(file_path, file_name):
    ...
    keywords.append(file_specific_keyword)
...
def process_remaining_files():
    """
    Process all remaining English documentation files in the main docs directory.
    """
    # Get all documentation files from the main docs directory only
    ...
    print(f"Found {len(all_files_sorted)} total documentation files in main docs directory")
    print(f"Previously processed: {len(processed_files)} files")
    print(f"English files available: {len(unprocessed_english_files)}")
    print(f"Processing all remaining: {len(unprocessed_english_files)} files")
    print("="*70)

    processed_count = 0
    skipped_count = 0
    
    for i, file_path in enumerate(unprocessed_english_files, 1):
        file_name = os.path.basename(file_path)
...
""")
    ...
    print(f"\nCompleted processing of {processed_count} files out of {len(unprocessed_english_files)} identified")
    print(f"Skipped {skipped_count} files due to errors")
    print(f"Total files processed in this run: {processed_count}")
    
    # Update the master tracker
    update_master_tracker(processed_count)


def update_master_tracker(processed_count):
    """Update the master tracker with the new progress."""
    tracker_path = "/common/active/sblo/Dev/FinCom/plan/manual_processing/master_tracker.md"
    ...
    for i, line in enumerate(lines):
        if line.startswith('- **Already Processed**:'):
            # Extract current count and add processed_count
            parts = line.split('(')
            current_count = int(parts[0].split(': ')[1])
            new_count = current_count + processed_count
            
            # Update the line
            lines[i] = f"- **Already Processed**: {new_count} (plus {new_count - 3} additional files from systematic batch processing)"
            
            # Update the completion rate
            total_files = 1164  # As per the tracker
            completion_rate = (new_count / total_files) * 100
            for j, line2 in enumerate(lines):
                if line2.startswith('- **Completion Rate**:'):
                    lines[j] = f"- **Completion Rate**: {completion_rate:.2f}%"
                    break
            break
    ...
    print(f"Updated master tracker with {processed_count} additional processed files")
