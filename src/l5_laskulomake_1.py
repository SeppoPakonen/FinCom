"""
Python implementation of L5_Laskulomake (1) invoice form template.

This module provides programmatic access to the invoice form functionality
originally implemented in the L5_Laskulomake (1).xlsx spreadsheet.
"""
...
class InvoiceForm:
    """
    A class representing an invoice form based on the L5_Laskulomake (1) template.
    This tool helps businesses create standardized invoices for customers, 
    detailing products or services provided, pricing, taxes, and payment terms.
    The template streamlines the invoicing process and ensures compliance with 
    Finnish invoicing regulations.
    """
    
    def __init__(self, company_name: str = "Invoice Company", year: int = 2026):
        ...
        self.late_payment_interest = 0.07  # Default 7% late payment interest
    ...
    def _initialize_defaults(self):
        """Initialize the invoice form with default values."""
    ...
    def calculate_totals(self) -> Dict:
        ...
        # Calculate percentage of total for each VAT rate
        for rate, data in vat_by_rate.items():
            data["percentage_of_total"] = (data["amount"] / total_vat * 100) if total_vat != 0 else 0
    ...
    def generate_reference_number(self) -> str:
        ...
        # For this example, we'll create a simple reference number
        import random
        self.reference_number = f"{20000 + len(self.invoice_items)}"
        return self.reference_number
    
    def generate_invoice_document(self) -> Dict:
        """Generate a complete invoice document."""
    ...
    def generate_invoice_report(self) -> Dict:
        """Generate a comprehensive invoice report with additional analytics."""
    ...
    def _generate_recommendations(self, invoice_doc: Dict) -> List[str]:
        """Generate recommendations based on the invoice."""
        recommendations = []
        ...
        if not recommendations:
            recommendations.append("Invoice appears to be in good order. Payment terms and VAT rates look appropriate.")
...
def create_sample_invoice() -> InvoiceForm:
    """Create a sample invoice with example data."""
    invoice = InvoiceForm(company_name="Sample Invoice Oy", year=2026)
    
    # Set sender information
    invoice.set_sender_info(
        name="Sample Invoice Oy",
        address="Laskutuskatu 15, 33100 Tampere",
        business_id="1234567-8",
        contact_person="Laskuttaja: Liisa Laskuttaja",
        phone="0500 123 456",
        email="liisa.laskuttaja@sampleinvoice.fi",
        website="www.sampleinvoice.fi",
    ...
    invoice.add_invoice_item(
        ...
        vat_percentage=24.0  # Standard Finnish VAT rate
    )
...
if __name__ == "__main__":
    # Example usage
    print("L5 Laskulomake (1) - Invoice Form Tool")
    print("=" * 45)
    ...
    # Save the invoice to a JSON file
    sample_invoice.save_to_json("sample_invoice_form.json")
    print(f"\nInvoice form saved to 'sample_invoice_form.json'")