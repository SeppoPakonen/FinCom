"""
Script to process the next 20 documentation files systematically.
...
def create_basic_processed_data(file_path, file_name):
    ...
    common_terms = [
        ...
        'digital', 'photography', 'budget', 'budgeting'
    ]
...
def process_next_20_files():
    """
    Process the next 20 documentation files systematically.
    """
    files_to_process = [
        "AB-Card.md",
        "ABC of Working Life Rules and Obligations.md",
        "AB-Kortti.md",
        "Articles of Association for a Housing Company to be Established.md",
        "Articles of Association for a Limited Liability Company Template.md",
        "Arviointilomake Liiketoiminnan tehtävissä toimiminen.md",
        "Arviointilomake Myynti ja markkinointi.md",
        "Arviointilomake tuotteiden ja palveluiden kehittäminen.md",
        "Arviointilomake Yritystoiminnan suunnittelu ja käynnistäminen.md",
        "Asiakaslupaus.md",
        "asiakaspalvelu kuluttajille.md",
        "asiakaspalvelu.md",
        "asiakirjastandardi_SFS_2487.md",
        "Assessment Form Developing Products and Services.md",
        "Assessment Form Planning and Starting Business Operations.md",
        "Assessment Form Sales and Marketing.md",
        "Assessment Form Working in Business Operations.md",
        "Associations.md",
        "Base Text Digital Photography.md",
        "budjetointi.md"
    ]
    
    print(f"Processing next 20 documentation files...")
    print("="*70)
    ...
    for i, file_name in enumerate(files_to_process, 1):
        ...
        file_path = f"../docs/business_forms/{file_name}"
        ...
        try:
            ...
            entities_count = len(processed_data["ecs_elements"]["entities"])
            components_count = len(processed_data["ecs_elements"]["components"])
            systems_count = len(processed_data["ecs_elements"]["systems"])
            constraints_count = len(processed_data["constraints"])
...
- **Keywords Extracted**: {keywords_count}
- **Entities Identified**: {entities_count}
- **Components Identified**: {components_count}
- **Systems Identified**: {systems_count}
- **Constraints Extracted**: {constraints_count}
- **Metadata Added**: Yes
""")
