"""
Script to process the next 50 documentation files systematically from the business_forms subdirectory.
...
def is_likely_english_content(file_path):
    """
    Check if a file has English content despite potentially having a Finnish filename.
    This function analyzes the actual content of the file to determine language.
    """
    try:
        with open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
            content = f.read()[:2000]  # Check first 2000 chars
        
        # Count Finnish vs English indicators in the content
        finnish_chars = ['ä', 'ö', 'å', 'Ä', 'Ö', 'Å']
        finnish_char_count = sum(1 for c in content if c in finnish_chars)
        
        # Count Finnish words that appear in business contexts
        finnish_words = [
            ...
            'perusteet', 'kilpailukeinot', 'täydennys', 'täsmennys', 'liikeidea', 'toimintamalli',
            'ammattilainen', 'kädenjälki', 'palvelumuotoilu', 'tuotteistaminen', 'asiakaspalvelu',
            'konsultointipalvelut', 'uusyrityskeskus', 'perustamisopas', 'verotietoa', 'yritystoiminnan',
            'aloittavan', 'liiketoimintasuunnitelma', 'hinnoittelu', 'kannattavuus', 'talous', 'taloussuunnitelma'
        ]
        ...
        for word in finnish_words:
            # Look for the word as a whole word, not as part of another word
            if re.search(r'\b' + re.escape(word.lower()) + r'\b', content_lower):
                finnish_word_matches += 1
        
        # Count English indicators in content
        english_indicators = [
            'the', 'and', 'for', 'to', 'of', 'in', 'with', 'by', 'from', 'as', 'is', 'are', 'was', 'were',
            'business', 'company', 'formation', 'finance', 'tax', 'compliance', 'registration', 'requirements',
            'process', 'procedure', 'document', 'form', 'template', 'guidance', 'information', 'instructions',
            'association', 'housing', 'liability', 'limited', 'cooperative', 'assessment', 'customer', 'service',
            'marketing', 'operations', 'planning', 'starting', 'working', 'digital', 'budget', 'insurance',
            'safety', 'security', 'guarantee', 'regulation', 'employment', 'salary', 'benefit', 'contract',
            'partnership', 'shareholder', 'report', 'application', 'notification', 'obligation', 'duty'
        ]
        
        english_word_count = sum(len(re.findall(r'\b' + re.escape(indicator) + r'\b', content_lower, re.IGNORECASE)) 
                                for indicator in english_indicators)
        
        # If there are significantly more English indicators than Finnish words, it's likely English content
        # Also consider the ratio of Finnish characters to total content
        total_chars = len(content)
        if total_chars > 0:
            finnish_char_ratio = finnish_char_count / total_chars
        else:
            finnish_char_ratio = 0
            
        # Decision logic:
        # If more than 15% of characters are Finnish chars AND more Finnish words than English indicators, it's Finnish
        if finnish_char_ratio > 0.15 and finnish_word_matches > english_word_count:
            return False
        # If we have significantly more English indicators than Finnish words, it's English
        elif english_word_count > finnish_word_matches * 2:
            return True
        # If equal amounts but more English indicators than Finnish words, it's likely English content
        elif english_word_count > finnish_word_matches:
            return True
...
def process_next_50_files():
    """
    Process the next 50 English documentation files systematically from the business_forms subdirectory.
    """
    # Get all documentation files from the business_forms subdirectory
    docs_dir = "/common/active/sblo/Dev/FinCom/docs/business_forms/"
    all_files = [os.path.join(docs_dir, f) for f in os.listdir(docs_dir) if f.endswith('.md')]
    all_files_sorted = sorted(all_files, key=lambda x: x.lower())
    
    # Get list of already processed files from the processed_docs directory
    processed_docs_dir = "/common/active/sblo/Dev/FinCom/processed_docs/"
    processed_files = []
    if os.path.exists(processed_docs_dir):
        processed_doc_files = os.listdir(processed_docs_dir)
        for pdf in processed_doc_files:
            if pdf.endswith('_processed.json'):
                # Convert processed filename back to original document name
                # Remove _processed.json and replace underscores with spaces
                original_doc_name = pdf.replace('_processed.json', '.md')
                processed_files.append(original_doc_name)
    else:
        processed_files = []
    ...
    for file_path in all_files_sorted:
        ...
        processed_name_variants = [
            file_name.replace('.md', '_processed.json'),
            file_name.replace(' ', '_').replace('-', '_').replace('.md', '_processed.json'),
            file_name.replace('-', '_').replace('.md', '_processed.json'),
            file_name.replace(' ', '_').replace('.md', '_processed.json')
        ]
        
        is_processed = False
        for variant in processed_name_variants:
            if os.path.exists(f"../processed_docs/{variant}"):
                is_processed = True
                break
    ...
    # Process next 50 English files
    files_to_process = []
    for file_path in unprocessed_files:
        if len(files_to_process) >= 50:
            break
        if is_likely_english_content(file_path):
            files_to_process.append(file_path)
    
    print(f"Found {len(all_files_sorted)} total documentation files in business_forms directory")
    print(f"Previously processed: {len(processed_files)} files")
    print(f"Unprocessed files: {len(unprocessed_files)}")
    print(f"English files available for processing: {len([f for f in unprocessed_files if is_likely_english_content(f)])}")
